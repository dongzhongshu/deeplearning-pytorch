...
def identity_block_test(target):
    ...
    X = torch.from_numpy(np.concatenate((X1, X2, X3), axis = 0).astype(np.float32))
    ...
    assert tuple(A3np.shape) == (3, 3, 4, 4), "Shapes does not match. This is really weird"
    ...
    A4 = target(X,
                f = 3,
                filters = [3, 3, 3],
                initializer=lambda seed=7:torch.tensor(1),
                training=True)
    A4np = A4.numpy()

...
# from torch.nn.functional import one_hot as to_categorical
...
def to_categorical(y, num_classes):
    ...
    a = np.eye(num_classes, dtype='uint8')
    return a[y]
...
def preprocess_data(dataset, human_vocab, machine_vocab, Tx, Ty):
    ...
    Xoh = np.array(list(map(lambda x: to_categorical(torch.tensor(x), num_classes=len(human_vocab)), X)))
    Yoh = np.array(list(map(lambda x: to_categorical(torch.tensor(x), num_classes=len(machine_vocab)), Y)))
